...
#     #####
# 	conv12 = Conv2D(2, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv11)
...
#model = BDLSTM_DenseUnet(opt, input_size=(256, 256, 1))
#model.summary()
...
# opt = SGD(lr=0.01, momentum=0.90, decay=1e-6)
# model_ = unet_3_out(opt, input_size = (256,256,1))
def unet_normal(opt, input_size = (256,256,1)):
    ...
    model.compile(optimizer=opt, loss=loss, metrics=[dsc], loss_weights=loss_weights)
    ######
...
def DenseUnet1(opt, input_size=(256, 256, 1)):
	...
	conv6, pool6 = transition(conv6, 3, n_f*32, dropout_rate=None,
                       weight_decay=1E-4, pool=False)
    # bottelneck #################################################################################################
	# conv7, nb_filter = denseblock(pool6, 3, 4,
 #                                  n_f*64, 12, 
 #                                  dropout_rate=None,
 #                                  weight_decay=1E-4)
	# conv7, pool7 = transition(conv7, 3, n_f*64, dropout_rate=None,
 #                       weight_decay=1E-4, pool=False)
	...
	conv11, pool11 = transition(conv11, 3, n_f*1, dropout_rate=None,
                       weight_decay=1E-4, pool=False)
    ########################################################
#	up1 = Conv2DTranspose(n_f*1, kernel_size=2, strides=2, padding='same',kernel_initializer = 'he_normal')(conv11)
#	up1 = BatchNormalization(axis=3)(up1)
#	up1 = Activation('relu')(up1)
#	
#	x1 = Reshape(target_shape=(np.int32(N), np.int32(N), n_f*1))(conv1)
#	x2 = Reshape(target_shape=(np.int32(N), np.int32(N), n_f*1))(up1)
#	merge11  = concatenate([x1,x2], axis = 3) 
#	#merge10 = ConvLSTM2D(filters = 16, kernel_size=(3, 3), padding='same', return_sequences = False, go_backwards = True,kernel_initializer = 'he_normal' )(merge10)
#            
#	conv12, nb_filter = denseblock(merge11, 3, 4,
#                                  n_f*1, 12, 
#                                  dropout_rate=None,
#                                  weight_decay=1E-4)
#	conv12, pool12 = transition(conv12, 3, n_f*1, dropout_rate=None,
#                       weight_decay=1E-4, pool=False)
    ########################################################
	out_1 =  Conv2D(2, (1, 1), activation='softmax', name='final_1')(conv11)
	out_2 =  Conv2D(2, (1, 1), activation='softmax', name='final_2')(conv11)
